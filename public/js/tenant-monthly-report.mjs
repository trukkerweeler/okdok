...
function getDateRange() {
  ...
  const end = document.getElementById("includeThroughToday").checked
...
async function generateReport() {
  ...
  document.getElementById("reportPeriod").textContent =
    `Rent period: ${formatMonth(start)}${document.getElementById("includeThroughToday").checked ? ` (receipts through ${formatDate(end)})` : ""}`;
...
function renderPayments(payments) {
  ...
  body.innerHTML = payments.length
    ? payments
        .map(
          (payment) =>
            `<tr><td>${formatDate(payment.payment_date)}</td><td>${escapeHtml(payment.invoice_number || "-")}</td><td>${formatMonth(payment.rent_period)}</td><td>${escapeHtml(payment.property_address || "-")}</td><td>${escapeHtml(payment.payment_method || "-")}</td><td>${escapeHtml(payment.reference_number || "-")}</td><td class="amount">${currency.format(Number(payment.amount_paid || 0))}</td></tr>`,
        )
        .join("")
    : '<tr><td colspan="7" class="text-center text-muted py-4">No receipts found for this period.</td></tr>';
  document.getElementById("reportTotal").textContent = currency.format(total);
  document.getElementById("reportNote").textContent = document.getElementById(
    "includeThroughToday",
  ).checked
    ? "This report includes receipts for this rent period, including any received after month end through today. Payments for other rent periods (e.g. a late prior-month payment) are excluded."
    : "This report includes receipts for this rent period only, dated through the end of the month. Payments for other rent periods (e.g. a late prior-month payment) are excluded.";
}
